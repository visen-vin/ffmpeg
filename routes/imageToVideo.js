...
module.exports = (app, upload) => {
  app.post('/api/image-to-video', upload.single('image'), async (req, res) => {
    ...
    const outputFilename = `step1_video_${crypto.randomBytes(8).toString('hex')}.mp4`;
    ...
    try {
      ...
      res.status(200).json({ message: 'Step 1 complete. Video created from image.', outputFilename });
    } catch (e) {
      res.status(500).json({ error: 'Failed during video creation.', details: e.message });
    } finally { fs.unlink(imagePath, () => {}); }
  });
};